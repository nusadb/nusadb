...
fn stream_command_tag(outcome: &StreamOutcome) -> String {
    match outcome {
        StreamOutcome::Rows { count, command, .. } => match command {
            ...
            RowsCommand::Fetch => format!("FETCH {count}"),
        },
...
impl Catalog for EngineCatalog<'_> {
    ...
    fn partition_key_columns(&self, table: &str) -> Result<Option<Vec<String>>, nusadb_sql::Error> {
        // The partition-key columns of a partitioned parent — which WHERE predicates can prune.
...
fn command_tag(result: &ExecutionResult) -> String {
    match result {
        ExecutionResult::Rows { rows, command, .. } => match command {
            ...
            RowsCommand::Fetch => format!("FETCH {}", rows.len()),
        },
        ...
        ExecutionResult::ViewDropped => "DROP VIEW".to_owned(),
        ExecutionResult::PolicyCreated => "CREATE POLICY".to_owned(),
        ExecutionResult::PolicyDropped => "DROP POLICY".to_owned(),
        ExecutionResult::TypeCreated => "CREATE TYPE".to_owned(),
        ExecutionResult::TypeDropped => "DROP TYPE".to_owned(),
        ExecutionResult::DomainCreated => "CREATE DOMAIN".to_owned(),
        ExecutionResult::DomainDropped => "DROP DOMAIN".to_owned(),
        ...
        ExecutionResult::CursorDeclared => "DECLARE CURSOR".to_owned(),
        ExecutionResult::CursorClosed => "CLOSE CURSOR".to_owned(),
...
fn value_to_field(value: Value) -> Option<Vec<u8>> {
    match value {
        ...
        Value::Geometry(ref g) => Some(nusadb_sql::geometry::format(g).into_bytes()),
