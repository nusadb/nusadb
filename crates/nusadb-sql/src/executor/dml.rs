...
/// `OVERRIDING USER VALUE`: a value supplied for a `GENERATED BY DEFAULT AS IDENTITY` column is
/// ignored — blank those cells so the column auto-fills from its sequence instead. A no-op unless the
/// mode is `UserValue`. (`GENERATED ALWAYS` columns are `Serial { always: true }` and untouched.)
fn apply_overriding_user_value(
    overriding: Option<ast::Overriding>,
    columns: &[usize],
    fills: &[Option<super::coldefault::ColumnFill>],
    value_rows: &mut [Vec<Option<ast::Value>>],
) {
    if !matches!(overriding, Some(ast::Overriding::UserValue)) {
        return;
    }
    for (target_pos, &table_ordinal) in columns.iter().enumerate() {
        if matches!(
            fills.get(table_ordinal),
            Some(Some(super::coldefault::ColumnFill::Serial {
                always: false,
                ..
            }))
        ) {
            for row in value_rows.iter_mut() {
                if let Some(cell) = row.get_mut(target_pos) {
                    *cell = None;
                }
            }
        }
...
/// Whether `OVERRIDING SYSTEM VALUE` was requested — then an explicit value for a `GENERATED ALWAYS`
/// identity column is allowed (its reject is skipped).
const fn overriding_system_value(overriding: Option<ast::Overriding>) -> bool {
    matches!(overriding, Some(ast::Overriding::SystemValue))
}
...
#[allow(
    ...
    reason = "the batch context plus the write-check predicates (RLS, view CHECK OPTION) and the \
              identity OVERRIDING mode; each is a distinct concern"
)]
...
fn insert_rows_with_unique(
    ...
    // `OVERRIDING USER VALUE`: blank supplied values for GENERATED BY DEFAULT identity columns so they
    // auto-fill from their sequence.
    let mut value_rows = value_rows;
    apply_overriding_user_value(overriding, columns, &fills, &mut value_rows);
    ...
    // `OVERRIDING SYSTEM VALUE` allows an explicit value for a GENERATED ALWAYS identity, so skip the
    // reject that otherwise forbids it.
    if !overriding_system_value(overriding) {
...
fn upsert_rows(
    ...
    let fills = super::coldefault::column_fills(table, engine, txn)?;
    let enum_info = enum_columns_info(table, engine, txn)?;
    // `OVERRIDING USER VALUE`: blank supplied values for GENERATED BY DEFAULT identity columns.
    let mut value_rows = value_rows;
    apply_overriding_user_value(overriding, columns, &fills, &mut value_rows);
    // Columns supplied a concrete value by some row (an explicit `DEFAULT` cell does not count).
    let any_explicit = explicitly_supplied_columns(columns, &value_rows);
    // `OVERRIDING SYSTEM VALUE` allows an explicit value for a GENERATED ALWAYS identity.
    if !overriding_system_value(overriding) {
        reject_explicit_identity_always(table, &fills, &any_explicit)?;
    }
    reject_explicit_generated(table, &fills, &any_explicit)?;
