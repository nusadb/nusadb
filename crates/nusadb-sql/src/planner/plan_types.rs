...
pub struct InsertPlan {
    ...
    /// `OVERRIDING {SYSTEM | USER} VALUE`, or `None` when absent. `SystemValue` lets an explicit value
    /// override a `GENERATED ALWAYS AS IDENTITY` column; `UserValue` makes a `GENERATED BY DEFAULT`
    /// identity ignore the supplied value and use its sequence.
    pub overriding: Option<crate::ast::Overriding>,
